...
import IntroCard from './VantageUCard';
import Row from 'react-bootstrap/Row';
import Col from 'react-bootstrap/Col';

function VantageUGroup({ cards, columns }) {
    ...
    <Row xs={4} md={4} className="g-4 intro-card-row">
      {rowCards.map((card, index) => (
            <Col key={index} className="intro-card-col">
                <IntroCard content={card.content} image={card.image} link={card.link} alt={card.alt} newTab={card.newTab} />
            </Col>
            ))}
    </Row>
    );
