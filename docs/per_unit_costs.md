...
To add additional metrics or change the value of a metric, update your source CSV file. Then, click **Edit** and upload the edited file. The new or updated metric is displayed in the table.
...
```bash
curl --request POST \
     --url http://api.vantage.sh/v2/business_metrics \
...
{
    "title": "Requests",
    "cost_report_tokens": ["rprt_12abc345def678gh"],
    "values": [
        {"date": "2024-02-01", "amount": 1512},
        {"date": "2024-02-02", "amount": 1816},
        {"date": "2024-02-03", "amount": 1236}
    ]
}
'
```
- The `title` parameter is a string that represents the business metric title (e.g., `Requests` or `CPUUtilization`).
- `cost_report_tokens` is a list of strings that represents one or more Cost Reports you want the business metric to be added to.
  :::tip
  You can access `cost_report_tokens` via the `/cost_reports` [endpoint](https://vantage.readme.io/reference/getcostreports). 
  :::
- The `values` parameter is a list that comprises the `date` and `amount` key/value pairs associated with each business metric data point.
...
If you need to update or add additional data to a business metric, send a `PUT` request to the `/business_metrics` endpoint. In the below example, an additional month of data is added to the business metric that was previously created. The request `url` includes the unique business metric `token`, received in the response from the `POST` call, to update the data.

```bash
curl --request PUT \
     --url http://api.vantage.sh/v2/business_metrics/bsnss_mtrc_ab12345cde67fg8 \
     --header 'accept: application/json' \
     --header 'authorization: Bearer <AUTH_TOKEN>' \
     --header 'content-type: application/json' \
     --data '
...
If you want to delete a business metric, send a `DELETE` request to the `/business_metrics` endpoint, and specify the `token` for the business metric you want to delete in the `url`. 
