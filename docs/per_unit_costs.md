...
Business metrics can be imported from other systems directly into Vantage and analyzed alongside cost data. You can create automatic import integrations with Amazon CloudWatch or manually import these metrics from a CSV file.

:::note
You must be an Owner to import unit metrics. Anyone with Editor permissions and above can assign them to a specific Cost Report. For more information, see the [Role-Based Access Control](/rbac) documentation.
:::
