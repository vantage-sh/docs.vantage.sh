---
title: Forecasting
description: For every [Cost Report](/cost_reports), Vantage generates a forecast that projects where your costs are expected to be by the end of the current month. You can also view projected costs, several months ahead. Vantage uses a time-series model to generate these forecasts and will run through all available data ingested into the platform on a per-service basis (e.g., EC2, RDS). These costs are forecasted into the future, taking into account seasonality or one-off spikes.
---
...
<Frame>
![A Cost Report with a forecast for the next 6 months and the mouse hovering over a future date point](https://assets.vantage.sh/docs/future-forecasts.png)
</Frame>
...
<Frame>
![A Cost Report with a forecast for the month and the mouse hovering over a future date point](https://assets.vantage.sh/docs/forecasting.png)
</Frame>
