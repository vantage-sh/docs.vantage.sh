...
description: The Vantage Kubernetes agent is the default, recommended configuration to ingest cost and usage data from Kubernetes clusters to Vantage. The agent is a Docker container that runs in your Kubernetes cluster. The agent collects metrics and uploads them to Vantage.
---
...
<Frame>
![Vantage Kubernetes agent architecture diagram](https://assets.vantage.sh/blog/announcing-the-official-kubernetes-integration/vantage-kubernetes-agent-architecture.png)
</Frame>
...
<Frame>
![GKE Autopilot filters on a Cost Report](https://assets.vantage.sh/docs/gke-autopilot.png)
</Frame>
...
- A [Vantage API token](/vantage_account#create-an-api-token) with READ and WRITE scopes enabled (it's recommended to use a [service token](/vantage_account#create-a-vantage-api-service-token) rather than a [personal access token](/vantage_account#create-a-vantage-api-personal-access-token))
...
<Steps>
    ...
    vka-vantage-kubernetes-agent-0   1/1     Running   0          54m                           READY   STATUS    RESTARTS   AGEvka-vantage-kubernetes-agent-0   1/1     Running   0          54m
    ```
    </Step>
    <Step>
...
<Frame>
![The Settings Clusters section with two sample clusters displayed along with the most recent version](https://assets.vantage.sh/docs/k8s-upgrade-agent.png)
</Frame>
...
If Persistent Volumes are not supported with your cluster, or if you prefer to centralize persistence, S3 is available as an alternative for agents deployed in AWS. See the [section below](/kubernetes_agent#configure-agent-for-s3-persistence) for details. If you require persistence to a different object store, contact [support@vantage.sh](/).
...
Persistent Metrics Recovery is enabled by default in Kubernetes Agent version v1.0.29. (See the [Upgrade the Agent](/kubernetes_agent#upgrade-the-agent) section for details on how to upgrade your agent to the latest version.)
</Info>
...
- Compresses the reports into a `.tar` archive and moves them to a backup location, either:
	...
	- An S3 bucket [you configure](/kubernetes_agent#optional-configure-agent-for-s3-persistence)
...
<Steps>
    ...
    - See the [Agent S3 persistence setup](/kubernetes_agent#optional-configure-agent-for-s3-persistence) section for details.
    <Tip>
    **TIP**
