...
id: cost_reports
title: Cost Reports
description: Create Cost Reports in Vantage to drill into your costs and filter on a number of dimensions.
keywords:
  - Cost Reports
---
...
Cost Reports include detailed filters to see all accrued costs that match certain conditions. By default, Vantage provides you with a Cost Report, named **All Resources**, that will show you all costs across all your cloud service accounts.

You can create as many Cost Reports as you'd like and filter your costs across a number of dimensions. As you create Cost Reports, they will be displayed on the [Overview](/overview) page. Each report will automatically generate a [forecast](/forecasting).
...
:::note
As costs can be duplicated across multiple tag keys, grouping by multiple tag keys is not supported.
:::

After a Cost Report has been grouped, each grouping is displayed as its own column in the table below the graph. On the graph itself, group labels are displayed with dashes between each grouping, such as `production - Acme - nat-123456`.
...
You can choose to toggle a Cost Report to not amortize upfront fees if you want to view the upfront fees on the day they were incurred.
...
If you notice your costs in Vantage differ from what you see in Cost Explorer, this could be for a number of reasons, including:
...
<details><summary>Monthly Costs by Service</summary>

- Create individual filters sets for each connected provider (e.g., All Costs from AWS **OR** Costs from MongoDB Atlas).
- Set **Group By** to **Service**.
- Set the date range to **Last 6 Months**, and set the binning to **Monthly**.
- Change the table to display costs **By Date**.
